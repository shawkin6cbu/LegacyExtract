...
def parse_any_legacy_contract_text(original_text_from_pdf: str) -> dict:
    ...
    def search_and_extract(pattern, text_to_search, group_index=1, default="", flags=re.DOTALL | re.IGNORECASE):
    ...
    data['SLR1REL1'], data['SLR1NAM2'] = '', ""
    data['SLR1CELL1'], data['SLR1EMAIL'], data['SLR1CELL2'], data['SLR1EMAIL2'] = "", "", "", ""
    data['PARCELID'] = ""
    ...
    if idx != -1:
        ...
        for token in re.split(r"\s+", after):            # walk tokens after anchor
            if token.count('/') > 4:                     # first token with >4 "/" ends block
                ...
                filtered = ''.join(
                    ch if (ch.isdigit() or ch == '/' or ch == '-' or ch == ' ') else ' '
                    for ch in token
                )
    ...
    for k_buyer_contact in ['BYR1ADR1', 'BYR1ADR2', 'BYR1CELL1', 'BYR1EMAIL', 'BYR1CELL2', 'BYR1EMAIL2']:
        data.setdefault(k_buyer_contact, "")
...
def create_legacy_contract_folder_structure(final_folder_path: str, extracted_data: dict, is_buyer_checked: bool, is_seller_checked: bool) -> tuple[str, bool]:
    ...
    try:
        ...
        os.makedirs(os.path.join(final_folder_path, "Emails"), exist_ok=True)
